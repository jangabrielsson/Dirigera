...
function QuickApp:defineClasses()
  ...
  local function IDF(_,_,_) end

  local function getBaseData(d) 
    local bd = {
      id = d.id,
      type =  d.deviceType or d.type,
      name = d.attributes.customName or "<noname>",
      attributes = d.attributes,
      capabilities = d.capabilities,
      canReceive = {},
      manufacturer = d.attributes.manufacturer or "",
      model = d.attributes.model or ""
    }
    for _,v in ipairs(d.capabilities.canReceive or {}) do bd.canReceive[v] = true end
    return bd
  end

  local ChildInitData = {
    light = function(d,bd,t) 
      local canReceive = bd.canReceive
      if canReceive.colorHue and ChildColorLight then
        t[d.id] = {
          name = bd.name, 
          type = 'com.fibaro.colorController',
          className = 'ChildColorLight',
          properties = {colorComponents = {red=0,green=0,blue=0,warmWhite=0}, manufacturer = bd.manufacturer, model = bd.model}
        }
      elseif canReceive.colorTemperature and ChildTempLight then
        t[d.id] = {
          name = bd.name,
          type = 'com.fibaro.multilevelSwitch',
          className = 'ChildTempLight',
          properties = {
            uiView = json.decode('[{"components":[{"name":"tempLabel","style":{"weight":"0.5"},"text":"TEMPERATURE","type":"label","visible":true},{"eventBinding":{"onChanged":[{"params":{"actionName":"UIAction","args":["onChanged","setTemperature","$event.value"]},"type":"deviceAction"}]},"max":"100","min":"0","name":"tempSlider","style":{"weight":"1.0"},"text":"","type":"slider","visible":true}],"style":{"weight":"1.0"},"type":"horizontal"}]'),
            manufacturer = bd.manufacturer, model = bd.model
          }
        }
      elseif canReceive.lightLevel and ChildDimmableLight then
        t[d.id] = {
          name = bd.name,
          type = 'com.fibaro.multilevelSwitch',
          className = 'ChildDimmableLight',
          properties = { uiView = {}, manufacturer = bd.manufacturer, model = bd.model }
        } 
      else
        ERRORF("Device %s can't recieve lightLevel, colorHue or colorTemperature",bd.name)
      end
    end,

    lightController = function(d,bd,t) end,
    gateway = function(d,bd,t) end,

    speaker = function(d,bd,t) 
      t[d.id] = {
        name = bd.name,
        type = 'com.fibaro.player',
        className = 'ChildSpeaker',
        properties = {manufacturer = bd.manufacturer, model = bd.model},
      }
    end,

    motionSensor = function(d,bd,t) 
      t[d.id] = {
        name = bd.name,
        type = 'com.fibaro.motionSensor',
        className = 'ChildMotionSensor',
    ...
    lightSensor = function(d,bd,t)
      if bd.name=="" then
        local r = t[d.relationId.."_1"]
        if r then bd.name = r.name.."(Temp)" end
      end
      t[d.id] = {
        name = bd.name,
        type = 'com.fibaro.lightSensor',
    ...
    openCloseSensor = function(d,bd,t)
      t[d.id] ={
        name = bd.name,
        type = 'com.fibaro.doorSensor',
        className = 'ChildDoorSensor',
    ...
    environmentSensor = function(d,bd,t)
      t[d.id.."_pm25"] = {
        name = bd.name.."(PM25)",
        type = 'com.fibaro.multilevelSensor',
        properties = {unit = " µg/m³", manufacturer = bd.manufacturer, model = bd.model},
        className = 'ChildPM25Sensor'
      }
      t[d.id.."_voc"] = {
        name = bd.name.."(tVOC)",
        type = 'com.fibaro.multilevelSensor',
        properties = {unit = " ppm", manufacturer = bd.manufacturer, model = bd.model},
        className = 'ChildVocSensor'
      }
      t[d.id.."humidity"] = {
        name = bd.name.."(Humidity)",
        type = 'com.fibaro.humiditySensor',
        properties = {manufacturer = bd.manufacturer, model = bd.model},
        className = 'ChildHumiditySensor'
      }
    ...
    outlet = function(d,bd,t)
      local interfaces = {}
      if bd.attributes.totalEnergyConsumed then 
        interfaces[#interfaces+1] = 'energy' 
      end
      if bd.attributes.currentActivePower then 
        interfaces[#interfaces+1] = 'power' 
      end
      if #interfaces == 0 then interfaces = nil end
      t[d.id] = {
        name = bd.name,
    ...
    blinds = function(d,children) end,
    waterSensor = function(d,children) end,
    airPurifier = function(d,children) end,
  }

  function DG:addDevice(d,tab)
    local bd = getBaseData(d);
    (ChildInitData[d.deviceType] or IDF)(d,bd,tab) 
  end
